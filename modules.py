...
class GGNN(nn.Module):
    ...
    def forward(self, annotation, A, word_mask):
        # word_mask: [batch_size x n_node x max_word_num_one_node x annotation_dim]
        ...
        if self.word_split:
            if self.pooling_type == 'max_pooling':
                ...
                annotation = annotation + word_mask
...
class SeqEncoder(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, n_layers=1):
        ...
        self.lstm = nn.LSTM(emb_size, hidden_size, n_layers, dropout=0, batch_first=True, bidirectional=False)
