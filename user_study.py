...
def test(config, model, device):
    ...
    # 先用训练好的模型跑出整个code base中所有code的向量
    data_path = args.data_path+args.dataset+'/'

    code_base_set = eval(config['dataset_name'])(config, data_path,
                                f_irs=config['all_ir'], max_node_num=config['n_node'])
    ...
    for batch in code_data_loader:
        # batch[0:4]: anno, adjmat, node_mask, word_mask
    ...
    # 再用训练好的模型跑user study挑选出的百来条query，这些query经过一定改写
    query_desc_set = eval(config['dataset_name'])(config, data_path,
    ...
    # 计算相似度，根据结果排序以及ground truth位置给出R@k以及MRR值
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', type=str, default='github_user', help='name of dataset.java, python')
